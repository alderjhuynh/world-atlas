...
A foreign artificer buying manastone by the crate tends to think of it, reasonably enough given how it's sold to them, as inert until spent: a charged shard, wired or set into a working, releases its stored potential and goes dark, no different in principle from any other stored resource. This is true of raw, uncut kyrite, and it is the entire basis of the export trade. It is not true of a properly worked **kinstone**, and the difference between the two is not one of degree. A shard of raw kyrite is potential without shape. A kinstone is potential that has been told, patiently, over hours of careful cutting, exactly what to become, and it will keep becoming that thing, precisely and repeatably, for a caster who has any right to hold it, long after an equivalent raw shard would have burned itself out doing something far cruder. Most foreign buyers have never seen a true kinstone. The ones who have, and who understood what they were looking at, tend not to talk about the price.
      ...
      {
        heading: "The Stone Itself",
        markdown: `
Kyrite forms in visible seams through the Kyresh Range and its foothills, some surface-exposed, glowing faintly even unworked, a soft color that varies seam to seam and has always been used locally to judge a seam's quality before a single shard is cut. The glow is not decorative; it is a rough, reliable proxy for how much ambient charge the stone has drawn in and settled, undisturbed, over however many centuries it took to form, and both Kyrai cutters and foreign concessions grade by it, for entirely different reasons.

| Glow | Local name | Typical foreign grade | Typical use |
|---|---|---|---|
| Pale, near-clear | *thinseam* | Third-grade | Minor enchanted tools, low-draw components |
| Steady amber | *fullseam* | Second-grade | Standard artificing stock, the bulk of the export trade |
| Deep, slow-pulsing | *heartseam* | First-grade | Large-scale workings, prohibitively expensive abroad |
| Faintly patterned, uneven glow | *marked stone* | Not formally graded; usually rejected by foreign buyers as flawed | The only grade of stone a kinstone can be cut from |

That last row is, in practice, the whole of the difference between the foreign trade and the Kyrai one. What a foreign assayer reads as a flaw, an irregular internal glow that won't grade cleanly, is, to a Kyrai cutter, the single sign that a shard is worth carving at all, since an evenly-glowing stone has nothing in it for a kin-pattern to catch on.
        `
      },
      {
...
Reactions to raw kyrite vary by the receiving tradition. Vhonic ley-casters, accustomed to power that already pools and can be drawn from a fixed convergence, use it easily and often, treating a good shard as simply a portable convergence. Witchlands windworkers, whose entire practice assumes charge that moves and refuses to settle, find kyrite actively unsettling to work with, a stone that holds charge rather than letting it pass reading, to trained breath-sense, like something that has forgotten how to let go; more than one hired Witchlands caster has declined a second contract that required handling it.
      ...
      {
        heading: "Kinwork: The Kyrai Craft",
        markdown: `
Kinwork is not cutting for grade. It is engraving, fine, deliberate linework carved directly along a shard's natural internal planes, following the irregular glow-pattern that marks a stone as worth carving rather than fighting it into a symmetrical export shape. A Kinworker reads a marked stone's pattern the way a bonewright reads bone grain, and the carving itself follows one of several dozen recognized **kin-patterns**, geometric sequences, passed down through apprenticeship.

What the carving does, functionally, is shape the stone's release the way a channel shapes water: instead of a raw, undirected burst, a finished kinstone releases its charge into one specific, precisely repeatable effect, a ward that holds exactly this shape of threat at bay, a tool-charm that does exactly this one task cleanly, a healer's stone that draws exactly the right kind of hurt out of a body and no other. A kinstone is, in the most useful sense, a spell that has been given a permanent physical body instead of being cast anew each time, and a well-made one will perform its single task reliably for a lifetime, long after an equivalent raw shard, spent on the same effect repeatedly by brute force, would have gone dark.

### The Ancestor-Rite

Carving alone does not finish a kinstone. Every kin-pattern is cut alongside a quiet, spoken invocation of the lineage's ancestors, not decoration but structural to the craft as the Kyrai understand it: a kinstone carved without the rite is, by every account, mechanically identical and functionally dead, releasing nothing, and Kyrai cutters describe the difference not as a missing ingredient but as a missing permission. A pattern cut correctly by someone outside the lineage, or by a lineage member who skips or garbles the rite, produces what's called an **unkinned stone**: carved, glowing faintly, and inert, a fact that has frustrated more than one foreign scholar who assumed, reasonably by their own tradition's logic, that the geometry alone should have been enough.

This is also, practically, the single reason Kinwork has never been successfully reproduced abroad despite generations of foreign attempts to copy recovered pattern-fragments. The lines can be traced. The rite cannot be faked convincingly enough to matter, and every documented foreign attempt to carve a working kinstone from a copied pattern has produced, without exception, an unkinned one.
        `
      },
      {
        heading: "What the Concessions Take, and What They Miss",
        markdown: `
Kyrai skill in fine kyrite-working was not lost to the conquest; it was simply put to different use without its meaning intact. The most technically demanding cutting and grading work in every major concession is still done overwhelmingly by enslaved Kyrai artisans, whose steady hands and trained eye for a seam's internal pattern are the entire practical foundation the export trade runs on, a fact no concession ledger has ever felt any need to acknowledge. What the concessions extract from that skill is precision: clean cuts, accurate grading, stone shaped efficiently for foreign specifications. What they have never once managed to extract, because they have largely never known to look for it, is Kinwork itself. A concession assayer grading for glow-purity has no vocabulary for a marked stone's irregular pattern beyond "flawed," and a cutter working under supervision, forced to shape a marked seam into a smooth, gradeable export piece, is, from a Kyrai perspective, being made to destroy the one thing that made the stone worth carving in the first place.

It does not always go undone anyway. Kyrai artisans in several concessions maintain a long, carefully guarded tradition of cutting a true kin-pattern into a piece destined for export, disguised well enough to pass an assayer's eye, and completing the rite in a whisper over a workbench a supervisor has no reason to suspect matters at all. Most of these pieces are simple protective marks, carried, by whatever route the piece takes after sale, back to family or lineage still held elsewhere on the coast. A smaller, far more carefully protected few are cut for the resistance directly: stones that read, to any caster who isn't the intended bearer, as an ordinary spent or flawed shard, and do something else entirely in the right hand.
        `
      },
      {
...
The interior's resistance effort draws directly on the same tradition. Raid-coordination relies on kinstones cut to answer only to a specific bearer or lineage, useless if intercepted; border wards along the margin between occupied coast and free interior are maintained, in large part, by kinstones renewed on a rotation the concessions have never managed to map, let alone disrupt; and escape routes into the interior are marked, where they're marked at all, not on any paper map an interrogation could produce, but in kin-patterns cut into trailside stone that read as natural mineral veining to anyone without the lineage-knowledge to recognize them as anything else.
      ...
      },
      {
        ...
        markdown: `
Kinwork is exacting, slow work, and it carries its own costs, distinct from stonelung, which remains a miner's and a concession-cutter's affliction rather than a carver's.

- **The cut cannot be undone.** A marked seam is carved once. A slipped line, a rushed rite, or a pattern applied to a stone whose internal glow doesn't actually support it produces an unkinned stone permanently, no different in outcome from a raw shard spent to nothing, and the source seam, often a rare marked find in the first place, is simply lost.
