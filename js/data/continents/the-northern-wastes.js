window.WORLD_DATA["continents"].push(
{
    id: "the-northern-wastes",
    title: "The Northern Wastes",
    subtitle: "The North",
    summary: "The arctic upper reach of the Crown's landmass, an expanse of ice, black rock, and wind-scoured tundra held loosely by a scatter of clan-based splinter tribes, with one improbable, flower-choked crescent tucked into its middle that stays ice-free and habitable year-round.",
    tags: ["the North","splinter tribes","the Bright","the Dark","longships","rune-sticks","the Vosarai","bay","ice"],

    sections: [
      {
        heading: "Overview",
        markdown: `
**The Northern Wastes**, or, in the clipped, mildly dismissive shorthand used by the crowned court a continent's length south of them, simply **the North**, treated grammatically the way a capital treats any frontier it doesn't have to think about: present on the map, absent from the budget. The Wastes make up the whole upper reach of the same landmass the reigning house calls **[the Crown](entry.html?c=kingdoms&id=the-crown)**, for the same crown that has sat on the same line of heads for longer than any Wastes-born clan bothers keeping count of; that southern realm, its courts, its wars, its long reach into [Kyre](entry.html?c=continents&id=kyre), is its own subject and isn't this entry's business. What concerns this entry is everything north of where the Crown's maps stop being drawn with any real confidence, an arctic expanse of ice, black rock, and wind-scoured tundra that no southern administration has ever governed so much as nominally claimed, and one improbable, flower-choked crescent tucked into the middle of it that the rest of the Wastes regards as something between a miracle and a rumor.

The Wastes are old, cold, and largely indifferent to the distinction between land and ice. The southern edge, where the tundra first gives way to permafrost proper, is brutal: frozen half the year, boggy and biting with insects the other half, growing nothing taller than scrub and lichen. Further north, the ground stops pretending to be arable at all, and the Wastes become what the rest of the world actually pictures when the name comes up: glacier-fields, wind-carved ice-barrens, and a coastline of black cliffs calving bergs into water cold enough to kill an unprepared swimmer in minutes. Almost nothing grows here without the sort of shelter the land itself is unwilling to provide, which is precisely what makes the [Vosarai](entry.html?c=peoples&id=vosarai) crescent as strange a fact of geography as it is a fact of culture.

The one substantial exception to the Wastes' general hostility to life is the **bay** the Vosarai and their neighbors both fish and sail: a deep, ice-free inlet that stays open year-round thanks to a warm current running up from the south, and that connects, eventually, to the ocean shipping lanes the rest of the world actually uses. Every settlement of any real size in the Wastes, tribal or otherwise, sits within a hard week's travel of that bay or one of its smaller sister-inlets, not out of preference so much as necessity: it is the only reliable source of open water, and therefore of fish, trade, and any contact with anyone who isn't also freezing.
        `
      },
      {
        heading: "Geological Background",
        markdown: `
The Wastes are old, cold, and largely indifferent to the distinction between land and ice. The southern edge, where the tundra first gives way to permafrost proper, is brutal: frozen half the year, boggy and biting with insects the other half, growing nothing taller than scrub and lichen. Further north, the ground stops pretending to be arable at all, and the Wastes become what the rest of the world actually pictures when the name comes up: glacier-fields, wind-carved ice-barrens, and a coastline of black cliffs calving bergs into water cold enough to kill an unprepared swimmer in minutes. Almost nothing grows here without the sort of shelter the land itself is unwilling to provide, which is precisely what makes the Vosarai crescent as strange a fact of geography as it is a fact of culture.

The one substantial exception to the Wastes' general hostility to life is the **bay** the Vosarai and their neighbors both fish and sail: a deep, ice-free inlet that stays open year-round thanks to a warm current running up from the south, and that connects, eventually, to the ocean shipping lanes the rest of the world actually uses. Every settlement of any real size in the Wastes, tribal or otherwise, sits within a hard week's travel of that bay or one of its smaller sister-inlets, not out of preference so much as necessity: it is the only reliable source of open water, and therefore of fish, trade, and any contact with anyone who isn't also freezing.
        `
      },
      {
        heading: "The Shelter and the Rest of It",
        markdown: `
Nearly everything habitable in the Wastes owes its habitability to shelter of one kind or another, and the crescent the Vosarai occupy, walled by mountains on its landward side and opening onto the bay, is simply the largest, most complete example of a pattern repeated in miniature up and down the coast. Smaller wind-breaks, valley-folds, and lee-side fjords host smaller settlements the same way, on a smaller scale, with correspondingly smaller and harsher lives to show for it. The Vosarai crescent is not unique in kind, only in degree: enough shelter, enough warmth, enough sun reflected off enough water to grow actual gardens, which is why it alone, among every settlement in the Wastes, has ever produced anything the rest of the world wanted to buy. Vosarai custom's entire *kol / not-kol* worldview, sheltered against exposed, reads, from outside, less like a philosophy invented in a vacuum and more like the single most obvious lesson the Wastes has to teach anyone who lives there: this is documented at length elsewhere and isn't repeated here.

Everyone else in the Wastes lives closer to *not-kol* than the Vosarai ever have to, and knows it.
        `
      },
      {
        heading: "Climate",
        markdown: `
The Wastes run on a solar calendar so extreme it barely resembles a normal year. Summer brings weeks of true, uninterrupted daylight, called locally, in most of the region's tongues, some variant of **the Bright**, during which travel, raiding, trade, and construction are all compressed into as short and frantic a window as possible before conditions turn. Winter brings the mirror opposite, **the Dark**, weeks or months, depending on latitude, of daylight reduced to a thin grey smear at midday or absent entirely, during which most tribal life turns fully inward: halls sealed, stores rationed, and a great deal of the region's storytelling tradition produced, since a long dark winter with nothing to do but wait it out is, by every account, where most of it comes from.

The aurora that regularly lights the winter sky, called by outsiders simply **the Wastelight**, is treated by nearly every tribe in the region as meaningfully alive in some sense, not divine exactly, but not inert weather either, closer to a vast, slow, half-attentive presence that notices the land beneath it without particularly caring about it. Reading the Wastelight for omens ahead of a raid, a hunt, or a birth is close to universal practice, though the specific interpretive tradition, which colors mean which fortune, varies clan to clan enough that two tribes reading the same sky on the same night will sometimes walk away with opposite conclusions and neither will think the other is lying, only wrong, the way any two diviners reading different signs might be.
        `
      },
      {
        heading: "The Splinter Tribes",
        markdown: `
Outside the crescent, the Wastes are held, loosely and unevenly, by a scatter of clan-based peoples [the Common Language](entry.html?c=languages&id=common) lumps together, not always accurately, as the **splinter tribes**, a name that reflects how the region looks on a map, dozens of small, isolated coastal and inland holds rather than any single nation, more than it reflects anything the tribes themselves would claim in common. No Wastes clan answers to another, and none has ever answered, in any binding sense, to the Crown, whose maps claim the whole territory as a northern march of the realm and whose actual administrative presence north of the permafrost line consists of a handful of fortified trading posts, garrisoned thinly, and generally left to work out their own arrangements with whichever clan happens to hold the nearest coastline.

### What the Tribes Share

Despite the real differences between them, in dialect, in specific ancestor-lore, in which coastal stretch or inland valley a given clan calls its own, the splinter tribes share enough in common that outsiders, not entirely wrongly, tend to describe the whole culture in a single breath:

- **Clan-hall living.** The basic social unit is the **hall**, an extended kin-group of some dozens to low hundreds sharing a single longhouse or cluster of them, built low, timber-and-turf or, on the coast, whale-bone-framed, and oriented for surviving the Long Dark together rather than for anything resembling privacy. A hall's authority rests with its **hall-father** or **hall-mother**, a title earned by demonstrated competence and the hall's own collective will rather than strict birthright, though the eldest capable child of a strong hall-parent is favored by custom, not by law.
- **Ice-craft and sea-craft.** Nearly every hall of any size maintains its own longships, hulls reinforced for ice-choked water and built shallow enough to be dragged over pack ice by hand when open water runs out, a skill and a vessel type found nowhere else in the world in quite the same form. Whaling, seal-hunting, and raiding along the coast are all conducted with the same ships, by the same crews, and a young Wastes-born adult's first real standing in a hall is usually earned on one of the three.
- **Oath and blood-price.** Disputes between halls are settled, when they can be, by a formal exchange of **oath-goods**, and a killing between halls, deliberate or not, is settled by a **blood-price** paid in furs, worked iron, or standing service, negotiated hall-to-hall with no outside authority to appeal to if the negotiation fails. A blood-feud left unsettled by price is understood, by every clan alike, to run until someone dies to close it or both halls simply agree, exhausted, to let it lapse.
- **The carved word.** Where the Vosarai keep record in dye and the [Rohai](entry.html?c=peoples&id=driftfolk) in knotted cord, the splinter tribes keep record in **rune-sticks**: short lengths of bone, antler, or hardwood, incised with a shared, if regionally inflected, system of angular marks used for oaths, boundary claims, and short genealogical tallies. A rune-stick is not considered binding on its own; it is a memory aid for a claim that must also be spoken aloud, by a living witness, to carry any real weight in a dispute, which keeps the tribes' actual legal culture closer to the Rohai's spoken-testimony habits than to anything a literate southern court would recognize as a written record.
- **Ancestor-halls and the drowned dead.** The dead of a hall are honored at a shared **ancestor-bench**, a carved wall or row of standing timbers inside or just outside the hall proper, one mark added per notable ancestor rather than per death, roughly comparable in social function, if not in form, to a Vosarai root-shrine or a Rohai Long Tally, though the tribes would find both of those comparisons a little too tidy for their own taste. A hall-member lost at sea and never recovered is marked differently, and more gravely, than one who dies ashore, on the widespread local belief that the sea does not give back what the Wastelight has already claimed for its own reasons.

### Named Peoples, in Brief

A handful of the larger, longer-standing tribal confederations are worth naming, though none of them constitute anything like a unified Wastes people and a full account of any one clan's own customs, sagas, and internal hall-politics is its own undertaking, not this entry's:

| People | Rough territory | Known for |
|---|---|---|
| **The Skarrfolk** | The western ice-coast, beyond easy reach of the bay | The most accomplished whalers in the Wastes; famously reluctant to raid, on the grounds that a hall fed well by the sea has no real need to |
| **The Vantrid** | The eastern fjord-country | The most frequent raiders along the Crown's own northern coastline, a fact the Crown's garrison-posts budget for, quietly, the same unglamorous way a Kyre concession budgets for resistance raids |
| **The Ashkelder** | The deep interior, well inland of any coast | The tribe most southern travelers never meet at all; herders of a hardy, long-haired ice-elk rather than sailors, and the source of most of the worked ivory and hide goods that do eventually reach southern markets |
| **The Holdren** | The mountain passes ringing the Vosarai crescent itself | The tribe with the longest, closest, and most complicated relationship to the crescent below |
        `
      },
      {
        heading: "The Crescent and Its Neighbors",
        markdown: `
The Vosarai crescent's own culture, language, and daily life are treated at length elsewhere and are not repeated here; what belongs in this entry is the crescent's *place* within the wider Wastes, since the Vosarai do not live in isolation from the tribes around them, however much their sheltered geography sometimes makes it look that way from the outside.

The **Holdren**, whose mountain passes form the very wall that keeps the crescent temperate, are the Vosarai's nearest and oldest neighbors, and the relationship between the two peoples is old enough that neither side describes it in simple terms. Holdren hunters and traders come down into the crescent's outer garden-terraces regularly, exchanging hide, worked iron, and dried ice-elk meat for dye, cloth, and the crescent's famously reliable stores of preserved food, a trade relationship stable enough that it predates any Crown administrative presence in the region at all. The Vosarai extend the Holdren a limited, informal version of *kol*, sheltered without being rooted, distinct from the roofed-not-rooted category reserved for harbor-traders and pidgin-speakers, since the Holdren are neither strangers passing through nor kin by any lineage account, but something the crescent has never quite had a tidy category for and has, by all evidence, stopped trying to force one onto.

The Crown's own relationship to the crescent is thinner and more transactional than either people's relationship to each other. The bay's status as the one reliable ice-free anchorage for hundreds of miles makes it strategically worth a nominal claim, and Crown maps duly show the crescent as a tributary holding of the northern march; in practice, this amounts to a small, seasonal customs post at the bay's mouth, garrisoned by a rotating handful of soldiers who would rather be posted almost anywhere else, and a modest, regularly renegotiated tribute paid more to keep Crown warships from bothering the harbor than out of any real sense of belonging to the realm claiming them. Vosarai elders asked directly whether the crescent considers itself part of the Crown tend to answer the way they'd answer almost anything else weighted with obligation: by pointing out what currently shelters them and what they currently shelter in turn, and letting the asker draw their own conclusion about which column a distant crown, one that has never once sent aid during a hard Long Dark, actually belongs in.
        `
      },
      {
        heading: "Trade",
        markdown: `
Beyond the crescent's dye and flowers, already documented, the wider Wastes export three things with any regularity: **furs and hide** from the Ashkelder herds and coastal seal-hunts, thick, cold-adapted, and prized in southern markets precisely because nothing bred or grown in a warmer climate matches them; **worked ivory**, carved from ice-elk antler and, more rarely and more valuably, from sea-beast tusk, in a distinctive angular style that southern buyers can usually identify on sight; and **bog-iron**, smelted in small, hall-run forges from ore drawn out of the tundra's iron-rich peat, tougher and more corrosion-resistant than most southern steel, and quietly sought after by southern smiths willing to pay the Wastes' asking price rather than admit their own foundries can't quite match it.

None of this trade moves through any single, Crown-administered channel. It moves hall to ship to buyer, negotiated locally, carried out through the bay or one of its smaller sister-inlets, and taxed by the Crown's customs post only to the modest degree any given captain judges worth paying rather than risking the alternative. The Wastes, in this respect as in most others, have never much needed the Crown's permission to keep doing what they were doing before anyone south of the permafrost line thought to claim the land at all.
        `
      },
      {
        heading: "Related Notes",
        markdown: `
- [The Vosarai, Mouth of Flowers](entry.html?c=peoples&id=vosarai) — The Vosarai crescent is the Wastes' one flower-choked exception to the general hostility to life
- [Vosarin](entry.html?c=languages&id=vosarin) — Vosarin is the language of the crescent's people
- [The Crown](entry.html?c=kingdoms&id=the-crown) — The Crown holds only a nominal, seasonal claim on the Wastes
- [The Driftfolk of the Shattered Reach](entry.html?c=peoples&id=driftfolk) — The Rohai's stateless, reckoning-based culture is a useful point of comparison
        `
      },
    ]
  }
);
